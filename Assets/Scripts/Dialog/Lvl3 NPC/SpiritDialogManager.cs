...
public class SpiritDialogManager : MonoBehaviour {
    ...
    public AudioClip dialogSpiritSound;
    ...
    {
        if (audioSource != null && dialogSpiritSound != null)
        {
            audioSource.PlayOneShot(dialogSpiritSound);
        }
        else
        {
            Debug.LogError("звука нет");
        ...
        if (dialogStart)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                if (currentMessageIndex == 7)
                {
                    ...
                    {
                        Debug.LogError("PlayableDirector не назначен в инспекторе!");
                    }
                    ...
                    StartCoroutine(EndDialogAfterDelay(1.5f));
                }
            }
        }
    }
