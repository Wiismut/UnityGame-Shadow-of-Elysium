...
public class Leshii_AnswerButtons : MonoBehaviour {
    public Button[] buttons;
    private string[] currentReplyTags;
    private Leshii_DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<Leshii_DialogStory>();
    ...
    private void ChangeAnswers(Leshii_DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogLeshii", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.Leshii_ChangeStory(currentReplyTags[button]);
}