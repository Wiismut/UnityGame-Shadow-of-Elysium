...
public class Deer_AnswerButtons : MonoBehaviour {
    public Button[] buttons;
    private string[] currentReplyTags;
    private Deer_DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<Deer_DialogStory>();
    ...
    private void ChangeAnswers(Deer_DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogDeer", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.Deer_ChangeStory(currentReplyTags[button]);
}