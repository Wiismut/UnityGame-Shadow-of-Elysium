...
public class Kitsune_AnswerButtons : MonoBehaviour {
    public Button[] buttons;

    private string[] currentReplyTags;
    private Kitsune_DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<Kitsune_DialogStory>();
    ...
    private void ChangeAnswers(Kitsune_DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogKitsune", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.Kitsune_ChangeStory(currentReplyTags[button]);
}