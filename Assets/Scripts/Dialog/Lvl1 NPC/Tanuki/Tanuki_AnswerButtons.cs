...
public class Tanuki_AnswerButtons : MonoBehaviour {
    public Button[] buttons;
    private string[] currentReplyTags;
    private Tanuki_DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<Tanuki_DialogStory>();
    ...
    private void ChangeAnswers(Tanuki_DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogTanuki", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.Tanuki_ChangeStory(currentReplyTags[button]);
}