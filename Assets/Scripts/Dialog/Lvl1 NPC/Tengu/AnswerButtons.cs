...
public class AnswerButtons : MonoBehaviour {
    ...
    private DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<DialogStory>();
    ...
    private void ChangeAnswers(DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogTengu", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.ChangeStory(currentReplyTags[button]);
}