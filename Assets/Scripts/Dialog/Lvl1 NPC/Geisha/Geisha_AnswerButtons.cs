...
public class Geisha_AnswerButtons : MonoBehaviour {
    public Button[] buttons;

    private string[] currentReplyTags;
    private Geisha_DialogStory dialogueStory;
    ...
    private void Start()
    {
        dialogueStory = GetComponent<Geisha_DialogStory>();
    ...
    private void ChangeAnswers(Geisha_DialogStory.Story story)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var localizedText = new LocalizedString("DialogGeisha", story.Answers[i].TextKey);
            int index = i;
    ...
    private void SendAnswer(int button) => dialogueStory.Geisha_ChangeStory(currentReplyTags[button]);
}